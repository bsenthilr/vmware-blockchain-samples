...
async function convertPublicToPrivate() {
    ...
    await privacy_wallet.convert_public_to_private(values.privacyContractAddress, values.getPrivateTokenAbi(), values.publicContractAddress, values.getPublicTokenAbi(),
...
async function convertPrivateToPublic() {
    ...
    let userId = await values.getUserId()
    await privacy_wallet.convert_private_to_public(values.publicContractAddress, values.getPublicTokenAbi(), values.getPrivateTokenAbi(), values.privacyContractAddress,
        undefined, userId, value, parseInt(lastAddedTxNum) + 1);
}
...
async function syncState(lastKnownTxNum) {
    await privacy_wallet.sync_state(lastKnownTxNum, values.getPrivateTokenAbi(), values.privacyContractAddress);
}
