...
1. A Privacy Wallet Service - The service runs responsible for handling the utt payload.
2. A Privacy Wallet Frontend Application - The frontend application responsible for talking with VMBC (using a web3 client). It can read/write from/to the contracts, update the results in the Privacy Wallet service and ask it to produce a new utt transaction based on the current state.
...
To simplify the on-boarding process we have encapsulated all the basic wallet-frontend logic in `privacy-wallet.js`. This allows a user to simply import and use it in the user's application, working with the privacy logic "out of the box".
...
## transfer(privacy_contract_abi, privacy_contract_address, ether_account_private_key, recipient_id, recipient_public_key, value) ⇒
...
## convert\_private\_to\_public(privacy_contract_abi, privacy_contract_address, tokens_contract_abi, tokens_contract_address, ether_account_private_key, user_id, value) ⇒
